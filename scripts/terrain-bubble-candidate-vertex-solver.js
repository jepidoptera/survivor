...
function parseArgs(argv) {
    ...
    if (!["shared", "low-to-high", "high-to-low", "raw"].includes(options.partitionOrder)) {
        throw new Error("--partition-order must be shared, low-to-high, high-to-low, or raw");
    }
    if (!["binary", "transition"].includes(options.solver)) {
        throw new Error("--solver must be binary or transition");
    }
...
function usage() {
    return [
        "Usage:",
        "  node scripts/terrain-bubble-candidate-vertex-solver.js score [options]",
        "  node scripts/terrain-bubble-candidate-vertex-solver.js suggest --example-id <id> [options]",
        "  node scripts/terrain-bubble-candidate-vertex-solver.js suggest --input <input-json> [options]",
        "",
        "Options:",
        ...
        "  --threshold <n>         Candidate inclusion probability threshold (default 0.5)",
        ...
        "  --solver <mode>         binary or transition (default binary)",
        "  --partition-order <o>   shared, raw, low-to-high, or high-to-low (default shared)",
...
function trainCandidateModel(examples) {
    ...
    for (const example of augmented) {
        for (const type of innerTerrainTypes(example.input)) {
            for (const baseRing of baselineRingsForTerrain(example.input, type)) {
                ...
                for (const candidate of candidates) {
                    const feature = candidateFeature(example.input, type, candidate, baseRing);
    ...
    return {
        schema: "terrain-bubble-candidate-vertex-model-v0",
        trainedExampleCount: examples.length,
        augmentedExampleCount: augmented.length,
        featureCount: featureStats.size,
        localComponentModel: trainLocalComponentModel(augmented),
        localTransitionModel: trainLocalTransitionModel(augmented),
...
function featureDecision(model, feature, threshold) {
    const stat = model.features.get(feature);
...
function generateCandidateVertexPolygons(input, model, options = {}) {
    if (model && model.localTransitionModel) {
        if ((options.partitionOrder || "shared") === "shared") {
            const shared = generateSharedBoundaryPolygons(input, model, options);
            if (shared.polygons.length > 0) return shared;
        }
        const solved = generateLocalTransitionPolygons(input, model.localTransitionModel, options);
        if (solved.polygons.length > 0) return solved;
    }
    ...
    for (const type of innerTerrainTypes(input)) {
        const baseRing = baselineRingForTerrain(input, type);
        if (baseRing.length < 3) continue;

        const baseKeys = new Set(baseRing.map(pointKey));
        const candidates = allowedCandidatesForTerrain(input, type);
        const selected = new Map();
        let learnedIncluded = 0;
        let learnedExcluded = 0;
        let fallbackIncluded = 0;

        for (const candidate of candidates) {
            const feature = candidateFeature(input, type, candidate, baseRing);
            const decision = featureDecision(model, feature, threshold);
            const key = pointKey(candidate.point);
            if (decision) {
                if (decision.include) {
                    selected.set(key, candidate.point);
                    learnedIncluded++;
                } else {
                    learnedExcluded++;
                }
        ...
        if (selected.size < 3) {
            for (const point of baseRing) selected.set(pointKey(point), point);
        }
        ...
        const points = normalizeRing(ordered);
...
function buildSuggestion(input, model, options = {}) {
    ...
    return {
        ...
        name: options.name || "candidate vertex suggestion",
        createdAt: now,
        updatedAt: now,
        ...
        editor: {
            edited: false,
            generated: true,
            generatedBy: "terrain-bubble-candidate-vertex-solver-v0",
            ...
            candidateVertexSolver: {
                ...
                componentObservations: model.localComponentModel ? model.localComponentModel.observationCount : 0,
                componentSignatureCount: model.localComponentModel ? model.localComponentModel.signatureCount : 0,
                transitionObservations: model.localTransitionModel ? model.localTransitionModel.observationCount : 0,
                transitionTableSizes: model.localTransitionModel ? model.localTransitionModel.tableSizes : {},
...
function scoreExamples(examples, model, options = {}) {
    ...
    return {
        schema: "terrain-bubble-candidate-vertex-score-v0",
...
function printScoreText(report, model, trainingExamples) {
    console.log("terrain bubble candidate vertex solver");
    ...
    console.log(`component observations: ${model.localComponentModel.observationCount}`);
    console.log(`component signatures: ${model.localComponentModel.signatureCount}`);
    console.log(`transition observations: ${model.localTransitionModel.observationCount}`);
    console.log(`transition exact keys: ${model.localTransitionModel.tableSizes.exact}`);
...
function main() {
        ...
        if (options.json) {
            console.log(JSON.stringify({
                model: {
                    ...
                    augmentedExampleCount: model.augmentedExampleCount,
                    featureCount: model.featureCount,
                    componentObservations: model.localComponentModel.observationCount,
                    componentSignatureCount: model.localComponentModel.signatureCount,
                    transitionObservations: model.localTransitionModel.observationCount,
                    transitionTableSizes: model.localTransitionModel.tableSizes
                },
    ...
    if (options.command === "suggest") {
        let input = null;
        let name = "candidate vertex suggestion";
        if (options.exampleId) {
            ...
            input = example.input;
            name = `${example.name || example.id} candidate vertex suggestion`;
        } else if (options.inputPath) {
            input = readInputOrExample(options.inputPath);
            name = `${path.basename(options.inputPath)} candidate vertex suggestion`;
        } else {
            throw new Error("suggest requires --example-id or --input");
        }
        const suggestion = buildSuggestion(input, model, {
            id: options.exampleId ? `${options.exampleId}-candidate-vertex-suggestion` : undefined,
            name,
            threshold: options.threshold,
            partitionOrder: options.partitionOrder,
            sharedOwner: options.sharedOwner,
