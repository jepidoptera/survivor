...
test("placing ground furniture keeps exact world position instead of snapping to floor node", () => {
    class Spell {
        constructor(x, y) {
            this.x = x;
            this.y = y;
    ...
    globalThis.wizard = {
        ...
        currentLayerBaseZ: 0,
        selectedPlaceableCategory: "furniture",
        selectedPlaceableTexturePath: "/assets/images/furniture/hexrug.png",
        selectedPlaceableScale: 1,
        selectedPlaceableScaleMin: 0.2,
        selectedPlaceableScaleMax: 5,
        selectedPlaceableRotation: 0,
    ...
    assert.equal(placed.length, 1);
    assert.equal(placed[0].x, 4.25);
    assert.equal(placed[0].y, 5.75);
    assert.equal(placed[0].node, floorNode);
    assert.equal(placed[0].surfaceId, "floor-a");
    assert.equal(placed[0].fragmentId, "fragment-a");
});
...
test("placing furniture while physically on upper level attaches to upper floor node", () => {
    class Spell {
        constructor(x, y) {
            this.x = x;
            this.y = y;
    ...
    class PlacedObject {
        constructor(location, map, options) {
            ...
            this.map = map;
            this.options = options;
            this.traversalLayer = Number.isFinite(options.traversalLayer) ? Math.round(options.traversalLayer) : 0;
            this.level = Number.isFinite(options.level) ? Math.round(options.level) : this.traversalLayer;
    ...
    const groundNodeObjects = [];
    const upperNodeObjects = [];
    const groundNode = {
        xindex: 7,
        yindex: 8,
        surfaceId: "ground-surface",
        fragmentId: "ground-fragment",
        addObject(object) {
            groundNodeObjects.push(object);
        }
    };
    const upperNode = {
        xindex: 7,
    ...
    const map = {
        ...
        getFloorNodeAtLayer(xindex, yindex, layer, options) {
            queriedLayers.push({ xindex, yindex, layer, options });
            return layer === 2 ? upperNode : null;
        },
        objects: []
    };
    ...
    assert.equal(placed[0].fragmentId, "upper-fragment");
    assert.equal(groundNodeObjects.length, 0);
    assert.equal(upperNodeObjects.length, 1);
