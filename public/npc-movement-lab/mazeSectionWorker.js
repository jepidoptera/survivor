...
function normalizeMazeOptions(raw) {
    ...
    if (!options) throw new Error("NPC movement lab maze worker requires options");
    return {
...
function normalizeSectionKeys(keys) {
    if (!Array.isArray(keys)) throw new Error("NPC movement lab maze worker requires section keys");
    return keys.map((key) => {
        const coord = parseMazeSectionKey(key);
...
function normalizeBounds(bounds) {
    if (!bounds || typeof bounds !== "object") throw new Error("NPC movement lab maze worker requires path bounds");
    return {
...
function normalizeWalls(walls, label) {
    if (!Array.isArray(walls)) throw new Error(`NPC movement lab maze worker ${label} must be an array`);
    return walls.map((wall, index) => {
        if (!wall || typeof wall !== "object") throw new Error(`NPC movement lab maze worker ${label}.${index} is missing`);
        return {
...
function parseMazeSectionKey(key) {
    ...
    if (!Number.isFinite(q) || !Number.isFinite(r)) throw new Error(`NPC movement lab maze section key is invalid: ${key}`);
    return { q, r };
}
...
function getMazeSharedHallConnection(q, r, side, options, requireOpen = false) {
    ...
    if (requireOpen && !open) throw new Error(`NPC movement lab maze hallway edge ${ordered} is not open`);
    return {
        side,
        edgeKey: ordered,
        open,
...
function getWallGapEndpoints(a, b, gapT, gapWidth) {
    const length = Math.hypot(b.x - a.x, b.y - a.y);
    if (!(length > gapWidth + 0.5)) throw new Error("NPC movement lab maze hallway gap requires a wall segment longer than the gap");
    const halfT = Math.max(0.02, gapWidth / length * 0.5);
    return {
...
function getWallGeometryApi() {
    ...
    if (!api || typeof api.connectionCrossesWallFaces !== "function") {
        throw new Error("NPC movement lab maze worker requires WallGeometry.connectionCrossesWallFaces");
    }
    return api;
}
...
function isPathfindingNodeTerrainPassable(node, walls, targetRadius) {
    if (!node || !Number.isFinite(node.x) || !Number.isFinite(node.y)) {
        throw new Error("NPC movement lab maze worker passability requires a finite node");
    }
    for (const wall of walls) {
        ...
        if (!Number.isFinite(nx) || !Number.isFinite(ny)) {
            throw new Error("NPC movement lab maze worker passability requires finite wall normals");
        }
